...
class Accelerometer(object):
    ...
    def __init__(self, i2cBus, gaussScale):
    	...
    	self.bus.write_byte_data(Accelerometer.LSM_ACC_ADDR, Accelerometer.CTRL_REG1_A,Accelerometer.POWER_ON)
    	self.bus.write_byte_data(Accelerometer.LSM_ACC_ADDR, Accelerometer.CTRL_REG4_A,gaussScale)
    	
    	if gaussScale == Accelerometer.SCALE_A_2G: 
    		self.scale = (-2.0/32768.0)
    	if gaussScale == Accelerometer.SCALE_A_4G: 
    		self.scale = (-4.0/32768.0)
    	if gaussScale == Accelerometer.SCALE_A_8G: 
    		self.scale = (-8.0/32768.0)
    	if gaussScale == Accelerometer.SCALE_A_16G: 
    ...
    def zero(self):
        ...
        while count < 100:
            ...
            print ('pitch = {0:.2f}, roll = {1:.2f}').format(orientation.pitch, orientation.roll)
    ...
    def getRawData(self):
    	x = 256 * self.bus.read_byte_data(Accelerometer.LSM_ACC_ADDR, Accelerometer.OUT_X_H_A) + self.bus.read_byte_data(Accelerometer.LSM_ACC_ADDR, Accelerometer.OUT_X_L_A)
    	if x >= 32768: 
    		x = BitArray(bin(x)).int
    
    	y = 256 * self.bus.read_byte_data(Accelerometer.LSM_ACC_ADDR, Accelerometer.OUT_Y_H_A) + self.bus.read_byte_data(Accelerometer.LSM_ACC_ADDR, Accelerometer.OUT_Y_L_A)
    	if y >= 32768: 
    		y = BitArray(bin(y)).int
    
    	z = 256 * self.bus.read_byte_data(Accelerometer.LSM_ACC_ADDR, Accelerometer.OUT_Z_H_A) + self.bus.read_byte_data(Accelerometer.LSM_ACC_ADDR, Accelerometer.OUT_Z_L_A)
    	if z >= 32768: 
...
class Gyroscope(object):
    ...
    def __init__ (self, i2cBus, dps):
        self.bus = SMBus(i2cBus)
        self.bus.write_byte_data(Gyroscope.L3GADDR, Gyroscope.CTREG1, Gyroscope.ON)
        self.bus.write_byte_data(Gyroscope.L3GADDR, Gyroscope.CTREG4, dps)
        if dps == Gyroscope.DPS250:
            self.scale = 250.0/32768.0
        if dps == Gyroscope.DPS500:
            self.scale = 500.0/32768.0
        if dps == Gyroscope.DPS2000:
    ...
    def zero(self):
        ...
        while count < 100:
            x, y, z = self.getRawData()
            count = 1 + count
            print ('x = {0:.2f}, y = {1:.2f}, z = {1:.2f}').format(x / self.scale, y / self.scale, z / self.scale)
            xtotal = x/self.scale - self.ZERO_X + xtotal
            ytotal = y/self.scale - self.ZERO_Y + ytotal
            ztotal = z/self.scale - self.ZERO_Z + ztotal
            time.sleep(.075)
    ...
    def getRawData(self):
        x = 256 * self.bus.read_byte_data(Gyroscope.L3GADDR, Gyroscope.XOUTHIGH) + self.bus.read_byte_data(Gyroscope.L3GADDR, Gyroscope.XOUTLOW)
        if x >= 32768:
            x = BitArray(bin(x)).int
            
        y = 256 * self.bus.read_byte_data(Gyroscope.L3GADDR, Gyroscope.YOUTHIGH) + self.bus.read_byte_data(Gyroscope.L3GADDR, Gyroscope.YOUTLOW)
        if y >= 32768:
            y = BitArray(bin(y)).int
            
        z = 256 * self.bus.read_byte_data(Gyroscope.L3GADDR, Gyroscope.ZOUTHIGH) + self.bus.read_byte_data(Gyroscope.L3GADDR, Gyroscope.ZOUTLOW)
        if z >= 32768:
            z = BitArray(bin(z)).int
            
        return self.scale * (x + Gyroscope.ZERO_X), self.scale * (y + Gyroscope.ZERO_Y), self.scale * (z + Gyroscope.ZERO_Z)
