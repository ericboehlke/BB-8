class KalmanFilter(object):
  	...
  	# it is rotating, this is called angular rate. (http://sensorwiki.org/doku.php/sensors/gyroscope)
  	...
  	# An accelerometer measures the forces acting on it including gravity and motion.
	...
	def filterX (self, accelerometerAngleX, gyroRateX, dt):
		self.filteredAngleX, self.biasX, self.PX = self.filter(accelerometerAngleX, gyroRateX, dt, self.filteredAngleX, self.biasX, self.PX)
		return self.filteredAngleX

	def filterY (self, accelerometerAngleY, gyroRateY, dt):
		self.filteredAngleY, self.biasY, self.PY = self.filter(accelerometerAngleY, gyroRateY, dt, self.filteredAngleY, self.biasY, self.PY)
